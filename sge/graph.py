...
class SubtaskGraph(abc.ABC):
    ...
    '''
    ind_to_id: Dict[subtask index:int -> subtask id:int]
    id_to_ind: Dict[subtask id:int -> subtask index:int]
...
class SubtaskGraphCache(SubtaskGraph):
    ...
    def set_graph_index(self, graph_index: Optional[int]):
        """Load a specific graph instance from the graph_list."""

        if graph_index is None:
            graph_index = np.random.choice(self.num_graph)
        else:
...
class SubtaskGraphGenerator(SubtaskGraph):
  ...
  def __init__(self,
               game_config,  # Mining or Playground object
               env_name: str,
               ):
  ...
  def _set_graph_config(self, level: int):
    """Initialize the dynamic, stationary configuration for the graph generator.
    This is not static config any more, must be called every time the graph
    is reset.

    Args:
      game_config: Mining or Playground object. TODO
      level: the difficulty preset. Corresponds to D1-D4 in (Sohn et al., 2018).
        Typically determines depth (layers, up to 6) and their range parameters.
  ...
  def reset_graph(self, graph_index=None, *,
    ...
    # TODO This is a dummy value that has no meaning. (used in step())
    self.graph_index = 1
    ...
    for bind in range(nbatch):
      ...
      for lind in range(self.nlayer-1):
        ...
        while i < na:
          ...
          code = graph_utils.batch_bin_encode_64(and_row)
        ...
        while i < no + ndt:
          ...
          or_row[ainds] = 1
          code = graph_utils.batch_bin_encode_64(or_row)
  ...
  @property
  def ind_to_id(self) -> Dict[int, int]:
    assert self.nbatch == 1
    return dict(enumerate(self._ind_to_id[0]))

  @property
  def id_to_ind(self) -> Dict[int, int]:
    assert self.nbatch == 1
    return dict(enumerate(self._id_to_ind[0]))
